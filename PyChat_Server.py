...
def leia_vaba_nimi(nimekiri, nimi): #Genereerib kasutamata kasutajanime
    for i in nimekiri:
        if i==nimi:
            nimi=nimi+"!"
            leia_vaba_nimi(nimekiri, nimi)
        else:
            continue
    return nimi
    ...
    def uus_ühendus(algatajanimi): #Siin tegeletakse iga uue inimesega, kes ühendub selle toaga
        ...
        for i in range(len(socketid)): #Moodustatakse järjend kasutajatest ja antakse ühendunutele teada, et on uus ühenduja
            ...
            try:
                ...
                socketid[i] = "PUUDUB"
        nimi = leia_vaba_nimi(kasutajad,nimi) #Ühendunule määratakse kasutamata nimi

        if "PUUDUB" in socketid: #Vabanetakse vajaduse korral kadunud socketist
                ...
                try:
                    ...
                    for n in range(len(socketid)):
                        try: # Anname märku, et keegi on vestlusest lahkunud
                            ...
                            pass
        if "PUUDUB" in socketid: #Korrastame socketite ja ühenduste järjendi
            for n in range(len(socketid)):
                try:
